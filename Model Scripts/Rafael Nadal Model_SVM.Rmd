...
```{r tsne, fig.height=5, fig.width=5, webgl=TRUE}
...
En el gráfico tridimensional de arriba podemos ver la representación en baja dimensión que obtenemos mediante la aplicación de T-SNE. Las observaciones rojas representan los partidas perdidos por Nadal, mientras que las verdes representan los ganados. Se puede ver que hay zonas donde predominan los rojos, zonas que son las que vamos a intentar identificar mediante la aplicación de SVM.
...
LLegamos hasta acá con una buena impresión de cómo lucen los datos. Ahora vamos a construir nuestro modelo predictivo aplicando Support Vector Machines.
...
Comenzamos con el ajuste de modelos.

```{r SVM}
#defino el train_contol
train_control <- trainControl(method = "repeatedcv",
                             number = 10,
                             repeats = 5)
#Armo una grid para hacer un gridsearch de parámetros
grid <- expand.grid(C = seq(4.9,5.1,0.1),
                    sigma = seq(0.01,0.017,0.0005))

#Separo data test y train
...
#Chequeo el mejor C y el mejor sigma que maximizan accuracy
svm.fit$bestTune

#Veo el modelo
svm.fit

#Pruebo sobre base de testeo
...
Con este último método tenemos la ventaja de que rápidamente podemos calcular la probabilidad de victoria y ajustar el umbral de clasificación para obtener mejores resultados predictivos.

# Conclusión

Los resultados predictivos que surgen de la aplicación de SVM a nuestro problema sin dudas son buenos. El accuracy calculado utilizando cross-validation es sin dudas muy bueno.
Sin embargo, viendo que los resultados obtenidos con Regresión Logística son muy parecidos, si tuvieramos que elegir entre ambos métodos podríamos tender a elegir el modelo de Regresión, ya que es una técnica mucho más fácil de comprender y con mayor capacidad explicativa.
